...
function App() {
  ...
  useEffect(() => {
    // Display a starting message when the site loads
    const welcomeMessage = {
      type: "system",
      text: "Welcome to Commit Message Quest! Type 'generate' to start your journey.",
    };
  ...
  const handleCommand = async () => {
    ...
    if (step === 0 && input.toLowerCase() === "generate") {
      ...
      setMessages((prevMessages) => [
        ...
        {
          type: "system",
          text: "Choose your class: [feat (Magician), fix (Warrior), chore (Archer)]",
        },
      ]);
      setStep(1);
    } else if (step === 1) {
      ...
      if (["feat", "fix", "chore"].includes(selectedClass)) {
        ...
        setMessages((prevMessages) => [
          ...prevMessages,
          { type: "user", text: `Selected Class: ${selectedClass}` },
        ]);
        setMessages((prevMessages) => [
          ...prevMessages,
          { type: "system", text: "Enter your commit message:" },
        ]);
        setStep(2);
      } else {
        setMessages((prevMessages) => [
          ...
          {
            type: "error",
            text: "Invalid class! Choose from [feat, fix, chore]",
          },
        ]);
      }
    } else if (step === 2) {
      ...
      try {
        ...
        if (res.ok) {
          ...
          setMessages((prevMessages) => [
            ...prevMessages,
            { type: "error", text: "Error generating commit message." },
          ]);
        }
      } catch (error) {
        setMessages((prevMessages) => [
